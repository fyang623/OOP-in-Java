...
public abstract class Graph {
	...
	public List<Integer> getOneHop(int v) {
		...
		for (Integer out : outNeighbors) {
			oneHop.add(out);
		}
		for (Integer in : inNeighbors) {
			oneHop.add(in);
		}
		return oneHop;
	}
	...
	public List<Integer> degreeSequence() {
		...
		for (int i=0; i<numVertices; i++) {
			temp = getNeighbors(i).size() + getInNeighbors(i).size();
			sequence.add(temp);
		}
		sort(sequence);
		return sequence;
	...
	private void sort(List<Integer> sequence) {
		for (int i=0; i<numVertices; i++) {
			int currInt = sequence.get(i);
			int pos = i;
			while (pos > 0 && sequence.get(pos) > sequence.get(pos-1)) {
				sequence.set(pos, sequence.get(pos-1));
				sequence.set(pos-1, currInt);
				currInt = sequence.get(pos-1);
				pos--;
			}
	...
	public static void main (String[] args) {
		GraphLoader.createIntersectionsFile("data/maps/CollegePark.map", "data/intersections/CollegePark.intersections");

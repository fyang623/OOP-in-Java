...
public class EarthquakeCityMap extends PApplet {
	...
	public void setup() {		
		// (1) Initializing canvas and map tile
		size(900, 700, OPENGL);
		if (offline) {
	...
	private void addKey() {	
		...
		text("Land Marker", 75, 120);
		text("Ocean Marker", 75, 140);
	...
	private void printQuakes() 
	{
		...
		for (Marker countryMarker : countryMarkers) {
			...
			for (Marker quake : quakeMarkers){
				if (quake.getStringProperty("country") == countryName)
					count++;
			}
			if (count != 0) 
				System.out.println(countryName + " : " + count);
			totalLandQuakes += count;
		}
		totalOceanQuakes = quakeMarkers.size() - totalLandQuakes;
		System.out.println("OceanQuakes : " + totalOceanQuakes);
	}

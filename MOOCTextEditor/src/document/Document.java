...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		for (int i=0; i<word.length(); i++) {
			if (newSyllable == false && vowels.indexOf(word.charAt(i)) >= 0) {
				newSyllable = true;
				count++;
			}
			else if (newSyllable == true && vowels.indexOf(word.charAt(i)) < 0) {
				newSyllable = false;
			}
		}
		if (count > 1 && word.substring(word.length()-2).matches("[^aeiouy]+e")) 
			count--;
	    return count;
	}
	...
	public double getFleschScore()
	{
		...
		float syllFound = getNumSyllables();
		float wordsFound = getNumWords();
		float sentFound = getNumSentences();
		double score = 206.835 - 1.015 * wordsFound/sentFound -84.6 * syllFound/wordsFound;
	    return (float) score;
	}

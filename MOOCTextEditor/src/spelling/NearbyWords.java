...
public class NearbyWords implements SpellingSuggest {
	...
	public void insertions(String s, List<String> currentList, boolean wordsOnly ) {
		...
		for(int index = 0; index <= s.length(); index++){
			for(int charCode = (int)'a'; charCode <= (int)'z'; charCode++) {
				...
				// if the item isn't in the list, isn't the original string, and
				// (if wordsOnly is true) is a real word, add to the list
				if ((!wordsOnly||dict.isWord(sb.toString())) && !currentList.contains(sb.toString())) {
					currentList.add(sb.toString());
				}
			}
		}
	...
	public List<String> suggestions(String word, int numSuggestions) {
		...
		word = word.trim();	
		if (word.isEmpty() || numSuggestions <= 0)
		...
		List<String> mutList = new LinkedList<String>();   // strings that are one modification away from the current string
		String curr = null;
		int searchCount = 0;
		while(!queue.isEmpty() && searchCount < THRESHOLD) {
			curr = queue.remove();
			mutList = distanceOne(curr, false);

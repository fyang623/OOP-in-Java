...
public class WPTree implements WordPath {
	...
	public List<String> findPath(String word1, String word2) 
	{
		...
		Dictionary dict = nw.getDict();
		if (!dict.isWord(word1) || !dict.isWord(word2))
		...
		List<String> mutList = null;
		queue.add(root);
		visited.add(word1);
		int searchCount = 0;
		while (!queue.isEmpty() && searchCount < THRESHOLD) {
			currNode = queue.remove();
			currWord = currNode.getWord();
			mutList = nw.distanceOne(currWord, true);
			for (String str : mutList) {
				if (!visited.contains(str) && dict.isWord(str)) {
					currNode.addChild(str);
					queue.add(new WPTreeNode(str, currNode));
					visited.add(str);
					if (str.equalsIgnoreCase(word2)) 
						return (new WPTreeNode(str, currNode)).buildPathToRoot();
				}
				searchCount++;
			}
		}
